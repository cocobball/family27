...
export default function NetworkModule({ ctx }) {
  ...
  // force re-render every second (timer UI)
  ...
  // ----- Actions -----
  async function setKids(state /* "on" | "off" */, minutes = 0, metaAction = null, bypassParent = false) {
    if (!bypassParent && !requireParentOrOpenModal()) return;
  ...
  useEffect(() => {
    const t = setInterval(async () => {
      ...
      if (cur?.allowUntil && ms <= 0 && !autoBlockRunningRef.current) {
        ...
        patch({ allowUntil: null });
        await setKids("off", 0, "AUTO_BLOCK_TIMER_EXPIRED", true);
  ...
  // Rewards/session integration
  useEffect(() => {
    const offStart = ctx.eventBus.on(NETWORK_EVENTS.SESSION_STARTED, async (p) => {
      // p.endsAt should be a timestamp or ISO. We'll accept both.
      const endsAtMs = typeof p?.endsAt === "number" ? p.endsAt : new Date(p?.endsAt).getTime();
      if (!Number.isFinite(endsAtMs)) return;

      const untilIso = new Date(endsAtMs).toISOString();

      // extend timer if later
      const curUntil = data?.allowUntil ? new Date(data.allowUntil).getTime() : 0;
      if (endsAtMs > curUntil) patch({ allowUntil: untilIso });

      // allow now (bypass parent)
      await setKids("on", 0, "SESSION_STARTED_ALLOW", true);
    });

    const offEnd = ctx.eventBus.on(NETWORK_EVENTS.SESSION_ENDED, async () => {
      const cur = ctx.store.get(defaultNetworkData());
      const ms = cur?.allowUntil ? remainingMs(cur.allowUntil) : 0;
      if (!cur?.allowUntil || ms <= 0) {
        patch({ allowUntil: null });
        await setKids("off", 0, "SESSION_ENDED_BLOCK", true);
      }
    });
    ...
    <div style={{ padding: 16, display: "grid", gap: 12 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
        <div>
          ...
          <div style={{ opacity: 0.8, marginTop: 4 }}>
            ...
            {" • "}
            Last check: {data?.lastStatusAt ? new Date(data.lastStatusAt).toLocaleTimeString() : "—"}
          </div>
        </div>
        ...
        <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid rgba(255,255,255,0.10)" }}>
          {allowActive ? (
            <div style={{ display: "grid", gap: 8 }}>
              ...
              <div style={ui.row}>
                <button disabled={busy} onClick={cancelTimer} style={{ ...ui.btn, ...(busy ? ui.btnDisabled : null) }}>
                  Cancel Timer
                </button>
                <button
                  disabled={busy}
            ...
            <div style={ui.subtle}>No timer running. Use timed allow below.</div>
          )}
        </div>
      </div>
      ...
      <div style={ui.card}>
        ...
        <div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
          <button disabled={busy} onClick={() => allowFor(30)} style={{ ...ui.btnPrimary, ...(busy ? ui.btnDisabled : null) }}>
            ON for 30 min
          </button>
          <button disabled={busy} onClick={() => allowFor(60)} style={{ ...ui.btnPrimary, ...(busy ? ui.btnDisabled : null) }}>
            ON for 60 min
          </button>
          <button disabled={busy} onClick={() => allowFor(120)} style={{ ...ui.btnPrimary, ...(busy ? ui.btnDisabled : null) }}>
            ON for 120 min
          </button>
        </div>
      </div>
      ...
      {showPw && (
        ...
        >
          ...
          >
            ...
            <div style={{ opacity: 0.85 }}>Enter parent password to control internet.</div>
